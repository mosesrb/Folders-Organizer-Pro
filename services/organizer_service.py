...
def sequential_rename(path: str, prefix: str, mode: str, sort_mode: str, dry_run: bool, filter_str: str, use_regex: bool, progress_callback):
    ...
    if mode == "files":
        items = [f for f in p.iterdir() if f.is_file() and f.name != '.organizer_history.json']
    else:
    ...
    for idx, item in enumerate(items):
        ...
        if not dry_run:
            ...
            final_path = new_path
            col_idx = 1
            # Check if exists AND is not the same file (handles case changes on Windows)
            while final_path.exists():
                # If it's the same file (same inode/path), it's a case-only rename, allowed.
                try:
                    if final_path.samefile(item):
                        break
                except: pass
            ...
            new_history.append({"action": "move", "src": str(item), "dst": str(final_path)})
